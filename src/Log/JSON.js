...
class JSONDecoder {
  ...
  reduce<x>(changeLog: ChangeLog<x>, buffer: x): x {
...
const decoder: Decoder<Op[]> = (log: Op[]) => new JSONDecoder(log)

export const encoder: Encoder<Op[]> = (changeList: ChangeList): Result<Op[]> =>
  changeList.reduce(changeLog, [])
